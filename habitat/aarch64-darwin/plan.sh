...
do_install() {
  ...
  build_line "Setting GEM_PATH=$GEM_HOME"
  export GEM_PATH="$GEM_HOME"
  gem install --local berkshelf-*.gem --no-document --ignore-dependencies
  ...
  if ! grep -q 'APPBUNDLER_ALLOW_RVM' "${pkg_prefix}/bin/berks"; then
    build_line "ERROR: binstub patch injection failed for ${pkg_prefix}/bin/berks"
    return 1
  fi

  build_line "** creating wrapper for runtime environment"
  mkdir -p "$pkg_prefix/libexec"
  mv "$pkg_prefix/bin/berks" "$pkg_prefix/libexec/berks"
  cat <<EOF > "$pkg_prefix/bin/berks"
#!/bin/bash
set -e
...
EOF
  chmod -v 755 "$pkg_prefix/bin/berks"
